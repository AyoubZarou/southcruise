...
django.setup()
from startup.models import Countries, CountryPerformance, Startup, PerformanceIndex, StrartupPerformance
models = [Countries, CountryPerformance, Startup, PerformanceIndex, StrartupPerformance]
for model in models:
...
for category in categories:
    ...
    for p in cat_csvs:
        df = pd.read_csv(os.path.join(cat_path, p), skiprows=3)
        df = df.set_index(['Country Code'])
        cols = df.columns.str.isnumeric()
        performance_index = PerformanceIndex(name=df['Indicator Name'].iloc[0], category=category)
